...
1. **The Victim Store's Phone Number**: Often listed on the store's public contact page or "About Us" section.
...
The platform allows an admin to have a phone number associated with their account. My initial thought was to simply register a new account with the victim's phone number, but this would immediately trigger an OTP to the victim's phone, which I do not control.

*I needed a way to ****associate**** the victim's phone number with an account I controlled, without the victim receiving the OTP.*

The critical flaw was found in the **admin invitation flow**. During the creation of a new admin account (via an invitation), I could set the phone number to the **victim's publicly listed phone number**.
...
The backend, seeing an admin account (the attacker's ) that was associated with the victim's phone number, and receiving a valid OTP, incorrectly authorized the request. The victim's store was instantly linked to the attacker's dashboard. A "Switch Store" button immediately appeared, allowing the attacker to seamlessly transition into the victim's admin panel with a single click.
...
1. **Create Attacker Admin**: Register a second admin account for the attacker via the invitation flow.

1. **Bind Phone Number**: During the second admin's profile setup, enter the `<VICTIM_PHONE>` as the account's phone number. Ensure the OTP is delivered to the attacker's email.

1. **Request OTP**: Using the second admin's session, send a request to `/admin/v2/otp/send` to get a new `<OTP_CODE>`.

1. **Link Store**: Send a `POST` request to `/admin/v2/users/accounts/connect` using the `<SECOND_ADMIN_JWT>`, the `<VICTIM_STORE_ID>`, and the `<OTP_CODE>`.

1. **Verify Takeover**: The victim's store dashboard is now linked to the attacker's account, granting full control.

## The Fix

The fix requires a fundamental change in the logic of the tenant linking flow:

- **Strict Phone Number Validation**: The platform must ensure that the phone number being used for linking is not only verified but is also unique and owned by the store owner being linked.

- **Two-Factor Authorization**: The tenant linking process should require a strong, non-bypassable second factor of authentication that is sent only to the original, trusted store owner's contact information, not to a newly created admin account.

- **Store-Specific OTP**: The OTP request in Step 4 should be tied to the store being linked, not just the admin account, and should be sent to the store's primary contact number, not an arbitrary number set during a secondary admin creation.
